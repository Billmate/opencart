    ...
    public function getMethod($address, $total) {        
        ...
        if ($status) {
            ...
            if ($billmate_fee[$countryData['iso_code_3']]['status']) {
                $title = sprintf($this->language->get('text_fee'),$billmate_invoice['SWE']['description'], $this->currency->format($this->tax->calculate($billmate_fee[$countryData['iso_code_3']]['fee'], $billmate_fee[$countryData['iso_code_3']]['tax_class_id']), '', ''), $this->tax->calculate($billmate_fee[$countryData['iso_code_3']]['fee'], $billmate_fee[$countryData['iso_code_3']]['tax_class_id']));
                
            } else {
                $title = sprintf($this->language->get('text_no_fee'),$billmate_invoice['SWE']['description'], $billmate_invoice['SWE']['merchant'], strtolower($countryData['iso_code_2']));
            }
